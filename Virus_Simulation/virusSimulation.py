...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        """
        num = random.random()
        if num < self.maxBirthProb * (1 - popDensity):
...
class Patient(object):
    ...
    def update(self):
        ...
        for v in self.viruses:
            try:
                newbie = v.reproduce(self.popDensity)
...
def runTrial(numViruses, maxPop, maxBirthProb, clearProb):
    ...
    populations = []
    for i in range(300):
        populations.append(patientZero.update())
    return populations
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    """
    # Instantiate patient and virus populations
    viruses = []
    for virus in range(numViruses):
        viruses.append(SimpleVirus(maxBirthProb, clearProb))
    patients = []
    ...
    # Commence 300 time-steps for patients and calculate avg virus pop at each step
    meanPopulations = []
    for timeStep in range(300):
        meanPopulations.append(0)
        patientNum = 1
        for patient in patients:
            meanPopulations[timeStep] += patient.update()
            patientNum += 1
    for timeStep in range(300):
        meanPopulations[timeStep] /= numTrials

    # Plot the results
    pylab.plot(meanPopulations, label="SimpleVirus")
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    # Run 150 untreated timesteps
    mean_raw_populations = []
    resistant_populations = []
    for timeStep in range(150):
        for patient in patients:
            try:
                mean_raw_populations[timeStep] += patient.update()
                resistant_populations[timeStep] += patient.getResistPop(['guttagonol'])
            except IndexError:
                mean_raw_populations.append(patient.update())
    ...
    # Calculate mean of trials
    for timeStep in range(300):
        mean_raw_populations[timeStep] /= numTrials
        resistant_populations[timeStep] /= numTrials


    # Plot trials
    pylab.figure()
    pylab.plot(mean_raw_populations, label="Total Pop")
    pylab.plot(resistant_populations, label="Resistant Pop")
    pylab.title("Resistant Virus Simulation with Patient Treatment at 150 Timesteps")
    pylab.xlabel("Time Steps")
    pylab.ylabel("Average Virus Population")
    pylab.legend(loc="best")
    pylab.show()

    print(mean_raw_populations)
